...
struct HomeView: View {
    ...
    @State private var selectedWorkoutID: Workout.ID? = nil
        ...
        NavigationStack {
            ZStack(alignment: .bottom) {
                ...
                if viewModel.isMenuOpen {
                    ...
                    SettingsView(isMenuOpen: $viewModel.isMenuOpen)
                        .transition(.move(edge: .trailing))
                        .zIndex(1)
                }
            }
            .navigationDestination(item: $selectedWorkoutID) { workoutID in
                if let index = viewModel.workouts.firstIndex(where: { $0.id == workoutID }) {
                    ExerciseView(workout: $viewModel.workouts[index])
                } else {
